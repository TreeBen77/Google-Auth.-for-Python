...
def generate(length, period=5):
  ...
  for loop in range(0, length):
      repeat = repeat + 1
      character = system_random.randint(2, 32)
      if not character < 7:
      ...
      token.token = f"{token.token}{character}"
  return token
...
class get():
  ...
  def totp(secret):
    return get.hotp(secret, intervals_no=int(time.time())//30)
